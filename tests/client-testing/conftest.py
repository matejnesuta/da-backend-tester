...
def testfiles_dir(request):
    """Get the testfiles directory path (ecosystems subdirectory)"""
    custom_dir = request.config.getoption("--testfiles-dir")
    if custom_dir:
        return Path(custom_dir)

    # Default to /testfiles/ecosystems if running in container, otherwise ./testfiles/ecosystems
    if Path("/testfiles/ecosystems").exists():
        default_testfiles = Path("/testfiles/ecosystems")
    elif (Path(__file__).parent / "testfiles" / "ecosystems").exists():
        default_testfiles = Path(__file__).parent / "testfiles" / "ecosystems"
    else:
